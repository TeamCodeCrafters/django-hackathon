...
class EquipeAnaliseViewSet(viewsets.ModelViewSet):
    ...
    def pendentes(self, request, *args, **kwargs):
        self.queryset = EquipeAnalise.objects.filter(status="pendente")
        return super().list(request, *args, **kwargs)

    @action(detail=False, methods=["get"])
    def aceites(self, request, *args, **kwargs):
        self.queryset = EquipeAnalise.objects.filter(status="aceita")
        return super().list(request, *args, **kwargs)
